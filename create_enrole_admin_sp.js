...
const spDefinition = `
DROP PROCEDURE IF EXISTS \`enroleCourseFromAdmin\`;
...
BEGIN
    ...
    ELSE
        ...
        -- Check for slot conflicts (simplified for recovery)
...
async function createSP() {
    ...
    try {
        ...
        // Since it contains DELIMITER and multiple statements, we should handle carefully.
        // But mysql2 multipleStatements: true allows running the DROP and CREATE if we strip DELIMITER.
        
        await connection.query("DROP PROCEDURE IF EXISTS `enroleCourseFromAdmin`;");
        
        // Remove DELIMITER and ;; for JS execution
        const cleanSP = spDefinition
            .replace(/DELIMITER ;;|DELIMITER ;/g, '')
            .replace(/END ;;|END;/g, 'END');

        await connection.query(cleanSP);
        console.log('Stored procedure enroleCourseFromAdmin created successfully.');
