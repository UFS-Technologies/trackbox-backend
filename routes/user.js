...
router.get('/Get_Dashboard_Metric_Details/:type', async (req, res, next) => {
  try {
    const rows = await user.Get_Dashboard_Metric_Details(req.params.type);
    res.json(rows);
  } catch (e) {
    res.status(500).json({ success: false, message: 'Failed to get dashboard metric details', error: e.message });
  }
});
